...
import { Box, Typography, Button, Card, CardContent, IconButton, Link, Tooltip, Avatar, CircularProgress } from "@mui/material";
...
const MyPage = () => {
    ...
    const fetchMyPageData = async () => {
      try {
        ...
        setSitterStatus({
          ...
          // 여기에 펫시터 정보 추가 (보통 별도 API 호출이 필요할 수 있음)
        });
        ...
        const mockPets = [
          {
            ...
            gender: "남아",
            weight: 25,
            profileImageUrl: petEx
          }
        ];
        setPets(mockPets);

        const mockSitterStatus = {
          registered: false
        };
        setSitterStatus(mockSitterStatus);
      }
    };
  ...
  const handleNicknameSave = async (newNickname) => {
    try {
      // 닉네임 변경 API 호출
      const response = await axios.patch('/api/member/nickname',
  ...
  const handleProfilePhotoUpload = async (e) => {
    ...
    try {
      ...
      // 프로필 이미지 업데이트 API 호출
      const updateResponse = await axios.patch('/api/member/profile-image',
        ...
        {pets.length > 0 ? (
          ...
          <Box sx={{ textAlign: 'center', py: 3 }}>
            ...
            <Button
              variant="outlined"
              onClick={handleAddPet}
              sx={{ mt: 1, borderColor: "#E9A260", color: "#E9A260" }}
            >
              반려동물 추가하기
            </Button>
          </Box>
